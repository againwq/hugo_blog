...
2. 一个BWP至少配置**24个RB**(大于SSB的20个) 
3. BWP的带宽必须大于等于SSB,但是BWP中不一定包含**SSB**
4. 对同一个UE来说,DL或UL**同一时刻**只能有**一个BWP处于激活状态**
...
如图所示的Case C, 在2.4GHz以内, 一个时隙**最多**发**4个SSB(OFDM 符号),** 在大于2.4GHz部分,**一个半帧**(5ms)**最多**发**8个SSB(OFDM 符号).**
